		...
		if (args.length > 0) {
			if (args.length > 1) {
				...
				if (sender.hasPermission("chestsql.public." + args[0])) {
					chestType = "Public";
					chestName = args[0];
					inventory = plugin.getServer().createInventory(null, 45, "远程箱子·公共·" + chestName);
				}
				else {
			...
			if (sender.hasPermission("chestsql.use.vip")) {
				inventory = plugin.getServer().createInventory(null, 45, "远程箱子·私有·" + chestName);
			}
			else {
				inventory = plugin.getServer().createInventory(null, 27, "远程箱子·私有·" + chestName);
			}
		}
