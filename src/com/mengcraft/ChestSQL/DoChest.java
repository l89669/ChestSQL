		...
		for (int i = 0; i < inventory.getSize(); i++) {
			...
			try {
				if (itemStacks[i] != null) {
					inventoryDataBuilder.append(StreamSerializer.getDefault().serializeItemStack(itemStacks[i]));
				}
				else {
					continue;
				}
			...
			if(result.last())	{
				...
				for(int i = 0; i < size; i++) {
					if (!itemDatas[i].equals("")) {
						itemStacks[i] = StreamSerializer.getDefault().deserializeItemStack(itemDatas[i]);
					}
					else {
						continue;
						}
